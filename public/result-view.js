...
function resetResultState() {
  ...
  host.hidden = true;
  host.innerHTML = '';
  form.elements.sampleMode.checked = false;
  status.textContent = '';
}
...
async function syncStudents() {
  ...
  try {
    ...
    status.textContent = data.students.length ? '' : 'No students found for the selected class.';
  } catch {
    if (requestVersion !== studentRequestVersion) return;
    options.students = [];
    select.innerHTML = '<option value="">Unable to load students. Please try again.</option>';
    select.disabled = true;
    status.textContent = 'Unable to load students. Please try again.';
