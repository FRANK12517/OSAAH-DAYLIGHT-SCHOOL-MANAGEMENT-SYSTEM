...
test('login gallery renders eleven responsive rows with accessible zoom controls', async () => {
  ...
  assert.equal((html.match(/loading="eager" decoding="async"/g) ?? []).length, 11);
  assert.equal((html.match(/width="\d+" height="\d+"/g) ?? []).length, 11);
  ...
  assert.match(html, /08-school-community-event\.png/);
  assert.match(html, /11-community-panel\.png/);
  assert.doesNotMatch(html, /06-school-fun-day\.jpg/);
  assert.ok(html.indexOf('id="login-gallery"') > html.indexOf('class="portal-grid"'));
  assert.match(css, /grid-template-rows:repeat\(11/);
