...
test('academic results persist terminal and mock scores with native Osaah IDs', async () => {
  ...
  const mockStudent = students.createStudent({ firstName: 'Ama', surname: 'Mock', classId: 'JHS 1', admissionYearId: '2026' });
  const mockTeacher = { ...teacher, assignedClassIds: ['JHS 1'] };
  const mockSubject = subjects.list({ classId: 'JHS 1' }, mockTeacher)[0];
  const mock = results.saveMockScore({ studentId: mockStudent.id, classId: 'JHS 1', subjectId: mockSubject.id, academicYear: '2026/2027', term: 'First Term', mockLabel: '1st Mock', totalScore: 65 }, mockTeacher);
