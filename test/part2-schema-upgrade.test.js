...
test('migration inventory remains uniquely versioned after fee structures v2 reconciliation', async () => {
  const migrations = await discoverMigrations(new URL('../schema', import.meta.url));
  assert.equal(migrations.length, 39);
  assert.equal(new Set(migrations.map((item) => item.version)).size, migrations.length);
  assert.equal(migrations.at(-1).name, '040_fee_structures_v2.sql');
});
