...
test('migration inventory remains uniquely versioned after payment and receipt reconciliation', async () => {
  const migrations = await discoverMigrations(new URL('../schema', import.meta.url));
  assert.equal(migrations.length, 43);
  assert.equal(new Set(migrations.map((item) => item.version)).size, migrations.length);
  assert.equal(migrations.at(-1).name, '044_fee_payments_receipts.sql');
});
