...
export function createAdmissionProspectusService({ now = () => new Date().toISOString(), schoolId = 'school-osaah-daylight', classes = ADMISSION_CLASSES, academicYears = [] } = {}) {
  ...
  function create(input, actor) { assertManager(actor); const classRecord = assertClass(input.classId); const yearRecord = assertYear(input.academicYearId, input.academicYear); const existing = findByCombination(classRecord.id, yearRecord.id); if (existing) throw new Error('A prospectus already exists for this class and academic year. Update or replace the existing record.'); const timestamp = now(); const record = { id: randomUUID(), schoolId, provenance: 'PRODUCTION', classId: classRecord.id, className: classRecord.name, academicYearId: yearRecord.id, academicYear: yearRecord.name, content: input.content ? sanitizeProspectusHtml(input.content) : null, documentUrl: null, documentName: null, documentType: null, documentSize: 0, status: 'DRAFT', isPublished: false, publishedAt: null, publishedBy: null, createdBy: actorId(actor), createdAt: timestamp, updatedBy: actorId(actor), updatedAt: timestamp }; records.set(record.id, record); audit('CREATED', record, actor); return publicRecord(record, true); }
