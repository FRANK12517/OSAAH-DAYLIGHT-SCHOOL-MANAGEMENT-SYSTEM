export function createAuditLog(input, clock = () => new Date().toISOString()) {
  ...
  return Object.freeze({ id: input.id ?? crypto.randomUUID(), schoolId: input.schoolId ?? null, userId: input.userId ?? null, roleId: input.roleId ?? null, action: input.action, entity: input.entity, entityId: input.entityId ?? null, transactionReference: input.transactionReference ?? null, previousValue: input.previousValue ?? null, newValue: input.newValue ?? null, ipAddress: input.ipAddress ?? null, sessionId: input.sessionId ?? null, deviceMetadata: input.deviceMetadata ?? null, occurredAt: clock() });
}
