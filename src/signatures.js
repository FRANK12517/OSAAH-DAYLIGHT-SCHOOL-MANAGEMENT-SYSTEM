...
const safeStorageKey = (value) => {
  ...
  return Boolean(key) && key.startsWith('signatures/') && !key.includes('..') && !/[\\\s]/.test(key) && !/^https?:/i.test(key);
};
...
export function createSignatureService({ now = () => new Date().toISOString(), schoolId = 'school-osaah-daylight', staff = null, classes = CORE_LEVELS } = {}) {
  ...
  function validate(input, actor) {
    ...
    const storageKey = text(input.storageKey ?? input.fileReference);
    if (!safeStorageKey(storageKey)) throw new Error('A valid secure signature reference is required.');
    ...
    if (signatoryRole === 'CLASS_TEACHER') {
      ...
      identity = teacherFor(teacherId);
    } else {
      identity = headteacherFor(actor);
    }
    const fullName = text(input?.fullName ?? identity?.fullName);
    const phone = text(input?.phone ?? identity?.phone);
    if (!fullName) throw new Error('Full name is required.');
    if (!phone) throw new Error('Phone number is required.');
    if (!isValidGhanaPhone(phone)) throw new Error('Invalid Ghana phone number.');
    return { signatoryRole, mimeType, size, storageKey, fullName, phone, academicYear: academicYear || null, term: term || null, classId: signatoryRole === 'CLASS_TEACHER' ? classId : null, teacherId: signatoryRole === 'CLASS_TEACHER' ? teacherId : identity?.id ?? null };
  }
  ...
  function upload(input, actor) {
    ...
    const record = { id: randomUUID(), schoolId, ...meta, name: meta.fullName, data: input.data ?? null, active: true, uploadedBy: actorId(actor), uploadedAt: now(), updatedAt: now() };
  ...
  function resolveForStudent(student, context = {}) {
    if (student?.schoolId && student.schoolId !== schoolId) throw new Error('Forbidden.');
  ...
  function options(actor) { assertActor(actor); const head = headteacherFor(actor); return { classes: classOptions(), teachers: activeStaff().filter((item) => item.roleKey === 'TEACHER').map((item) => ({ id: item.id, name: item.fullName, phone: item.phone ?? null })), headteacher: head ? { id: head.id, name: head.fullName, phone: head.phone ?? null } : null }; }
