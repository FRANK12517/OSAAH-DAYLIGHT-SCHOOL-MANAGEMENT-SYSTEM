...
export function createFinancialIntelligenceService({ fees, productionDataGuard = createProductionDataGuard({ environment: process.env.NODE_ENV }), dataQualityGuard = createAIDataQualityGuard(), schoolContextService = null, explain = null, now = () => new Date().toISOString() } = {}) {
  ...
  async function snapshot({ actor, academicYearId = null, termId = null, classId = null, studentId = null, includeExplanation = true } = {}) {
    ...
    const scoped = Object.fromEntries(Object.entries(guarded).map(([name, result]) => [name, result.records.filter((record) => periodMatches(record, period) && (!classId || record.classId === classId) && (!studentId || record.studentId === studentId || record.permanentStudentId === studentId))]));
    ...
    let explanation = null; if (includeExplanation && explain) { const key = JSON.stringify([base.revision, period.academicYearId, period.termId, classId, studentId, assessment.quality.status]); if (!narrationCache.has(key)) { try { narrationCache.set(key, await explain(publicSnapshot(base))); } catch { narrationCache.set(key, null); } } explanation = narrationCache.get(key); }
