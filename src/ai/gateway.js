...
import { AIOrchestratorError } from './orchestrator.js';
...
const ERRORS = Object.freeze({
  ...
  PROVIDER_UNAVAILABLE: [503, 'No AI provider is configured.'], ORCHESTRATION_TIMEOUT: [504, 'The AI request exceeded its execution limit.'],
  ORCHESTRATION_LIMIT: [422, 'The AI request exceeded its orchestration limit.']
});
...
function mapped(error, ids) {
  ...
  if (error instanceof AIOrchestratorError) return new AIGatewayError(['ORCHESTRATION_TIMEOUT', 'ORCHESTRATION_LIMIT', 'WRITE_DISABLED'].includes(error.code) ? error.code : 'INVALID_REQUEST', { ...ids, cause: error });
...
export function createAIGateway({ capabilityRegistry, toolRegistry, schoolContextService, productionDataGuard, dataQualityGuard, auditLogger, toolExecutor = executeAuthorizedAITool, providerRegistry = null, providerId = null, orchestrator = null } = {}) {
  ...
  async function execute({ authenticatedUser, capabilityId, capabilityIds = [], toolName, input = {}, query = null, requestId = crypto.randomUUID(), correlationId = requestId, providerRequired = false } = {}) {
    ...
    try {
      ...
      if (!input || typeof input !== 'object' || Array.isArray(input)) throw new AIGatewayError('INVALID_REQUEST', ids);
      const orchestrationRequested = Boolean(orchestrator && !toolName); const requestedCapabilities = [...new Set([capabilityId, ...capabilityIds].filter(Boolean))];
      if (orchestrationRequested && (typeof query !== 'string' || !query.trim() || !requestedCapabilities.length) || !orchestrationRequested && (typeof capabilityId !== 'string' || !capabilityId || typeof toolName !== 'string' || !toolName)) throw new AIGatewayError('INVALID_REQUEST', ids);
      ...
      if (orchestrationRequested) {
        for (const requestedCapability of requestedCapabilities) authorization.authorizeCapability(context, requestedCapability);
        const result = await orchestrator.execute({ authenticatedUser, authorizationContext: context, schoolContext, capabilityIds: requestedCapabilities, query, requestId, correlationId });
        await audit({ eventType: 'AI_GATEWAY_COMPLETED', requestStatus: 'COMPLETED', dataQualityStatus: result.grounding.quality?.status ?? null, productionDataOnly: true, metadata: { warningCount: result.grounding.warnings.length } }); return result;
      }
      ...
      await audit({ eventType: 'AI_GATEWAY_REJECTED', severity: ['INVALID_REQUEST', 'CAPABILITY_UNAVAILABLE', 'DATA_UNAVAILABLE', 'PROVIDER_UNAVAILABLE', 'ORCHESTRATION_TIMEOUT', 'ORCHESTRATION_LIMIT'].includes(error.code) ? 'WARNING' : 'SECURITY', requestStatus: error.code === 'WRITE_DISABLED' ? 'BLOCKED' : 'DENIED', authorizationResult: 'DENIED', errorCode: error.code });
      throw error;
    }
  }
