...
export function createAIProviderRegistry(initial = []) {
  ...
  async function invoke(providerId, operation, request) {
    ...
    const item = select(providerId), safeRequest = validateAIProviderRequest(request); const limitedRequest = Object.freeze({ ...safeRequest, maxOutputTokens: Math.min(safeRequest.maxOutputTokens ?? item.maxOutputTokens, item.maxOutputTokens) }); const started = performance.now();
