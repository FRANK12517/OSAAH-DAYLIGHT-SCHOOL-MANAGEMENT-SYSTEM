...
export function createConfiguredAIProviderRegistry({ env = process.env, fetchImpl = globalThis.fetch } = {}) {
  ...
  const isEnabled = enabled(env.OSAAH_AI_PROVIDER_ENABLED), timeoutMs = integer(env.OSAAH_AI_PROVIDER_TIMEOUT_MS, 30000, 'OSAAH_AI_PROVIDER_TIMEOUT_MS', { minimum: 1 });
  ...
  return Object.freeze({ registry, providerId, enabled: isEnabled });
}
