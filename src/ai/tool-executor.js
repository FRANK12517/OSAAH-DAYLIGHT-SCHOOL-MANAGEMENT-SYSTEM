...
export async function executeAuthorizedAITool({ authenticatedUser, toolName, input = {}, capabilityRegistry, toolRegistry, productionDataGuard, auditLogger = createAIAuditLogger(), requestId = crypto.randomUUID(), correlationId = requestId, additionalCapabilityIds = [] }) {
  ...
  try {
    ...
    const result = Object.freeze({ data: minimize(value, authorization.tool.outputSchema), quality: guarded?.quality ?? raw?.quality ?? null, productionData: guarded ? guarded.diagnostics : null, requestId, correlationId, authorization: Object.freeze({ capabilityId: authorization.capability.id, toolName: authorization.tool.name, scope: authorization.scope }) });
