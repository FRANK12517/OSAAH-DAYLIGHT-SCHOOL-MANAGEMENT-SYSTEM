...
const VALID_FINANCIAL_STATUSES = new Set(['VALID', 'POSTED']);
...
export function createReportingService({ now = () => new Date().toISOString(), schoolId = 'school-osaah-daylight' } = {}) {
  ...
  function buildFinancialReport(filters = {}, user = {}, feeds = {}) {
    ...
    const allInvoices = feeds.fees?.listInvoices?.({ requestedSchoolId: schoolId }) ?? [];
    const allPayments = feeds.fees?.listPayments?.() ?? [];
    const withinDates = (item) => { const value = item.date ?? item.transactionDate; return (!dateFrom || value >= dateFrom) && (!dateTo || value <= dateTo); };
    ...
    const matchClass = (item) => !classId || item.classId === classId;
    const matchStudent = (item) => !studentId || item.studentId === studentId || item.permanentStudentId === studentId;
    const invoices = allInvoices.filter((item) => withinDates(item) && matchAcademic(item) && matchTerm(item) && matchClass(item) && matchStudent(item));
    const payments = allPayments.filter((payment) => payment.schoolId === schoolId && payment.status === 'VALID' && withinDates(payment) && matchAcademic(payment) && matchTerm(payment) && matchClass(payment) && matchStudent(payment));
  ...
  async function buildFinancialReportAsync(filters = {}, user = {}, feeds = {}) { if (!feeds.generalFinance) return buildFinancialReport(filters, user, feeds); const [incomes, expenses] = await Promise.all([feeds.generalFinance.income.list(filters, user), feeds.generalFinance.expenses.list(filters, user)]); const feeFeed = { ...(feeds.fees ?? {}), listIncomes: () => incomes, listExpenses: () => expenses }; return buildFinancialReport(filters, user, { ...feeds, fees: feeFeed }); }
  ...
  return { buildReport, buildAcademicReport, buildFinancialReport, buildFinancialReportAsync, exportReport, generateOfficialDocument, verifyOfficialDocument, listDocuments, terms: () => [...TERMS], auditTrail: () => reportAudit.map((event) => ({ ...event })), availableAcademicYears, counts: () => ({ audits: reportAudit.length, officialDocuments: officialDocuments.length }) };
}
