...
export function createResultSlipPdfService() {
  async function pdf(result) {
    ...
    return new Promise((resolve, reject) => {
      ...
      document.moveDown(.45); line('Student Name', result.studentName); line('Permanent Student ID / OSAAH Student Index', result.studentIndexNumber); line('Class', result.classId); line('Academic Year', result.academicYear); line('Term', result.term); line(result.resultType === 'MOCK' ? 'Mock Examination' : 'Examination', result.resultType === 'MOCK' ? result.mockLabel : 'End-of-Term');
      section('SUBJECT RESULTS');
      const columns = [['SUBJECT', 145], ['SCORE', 52], ['GRADE', 48], ['POSITION', 62], ['REMARK', 175]];
