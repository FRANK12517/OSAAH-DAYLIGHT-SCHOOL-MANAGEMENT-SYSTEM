...
export function createReceiptBrandingService({ fees, students, schoolId = 'school-osaah-daylight' } = {}) {
  ...
  async function pdf(receipt) { const [headerFile, watermarkFile, fontFile] = await Promise.all([readFile(join(root, RECEIPT_HEADER_ASSET)), readFile(join(root, RECEIPT_WATERMARK_ASSET)), readFile(join(root, RECEIPT_FONT_ASSET))]); return makeUnicodePdf(receipt, headerFile, watermarkFile, fontFile); }
  return { get, markup, pdf, assets: { header: RECEIPT_HEADER_ASSET, watermark: RECEIPT_WATERMARK_ASSET, font: RECEIPT_FONT_ASSET } };
}
