...
function money(value) { return `GH₵ ${Number(value ?? 0).toFixed(2)}`; }
...
export function createReceiptBrandingService({ fees, students, schoolId = 'school-osaah-daylight' } = {}) {
  ...
  async function pdf(receipt) { const [headerFile, watermarkFile] = await Promise.all([readFile(join(root, RECEIPT_HEADER_ASSET)), readFile(join(root, RECEIPT_WATERMARK_ASSET))]); const header = pngRgb(headerFile); const watermark = pngRgb(watermarkFile); watermark.alpha = Buffer.from(watermark.alpha.map((value) => Math.round(value * 0.10))); header.rgb = deflateSync(header.rgb); header.alpha = deflateSync(header.alpha); watermark.rgb = deflateSync(watermark.rgb); watermark.alpha = deflateSync(watermark.alpha); return makePdf(receipt, header, watermark); }
  return { get, markup, pdf, assets: { header: RECEIPT_HEADER_ASSET, watermark: RECEIPT_WATERMARK_ASSET } };
}
