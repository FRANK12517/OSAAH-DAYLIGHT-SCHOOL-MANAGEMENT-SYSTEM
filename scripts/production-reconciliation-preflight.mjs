...
if (!process.env.DATABASE_URL) { process.stdout.write(`${JSON.stringify({ ok: false, error: { code: 'DATABASE_URL_MISSING' } })}\n`); process.exitCode = 1; }
  ...
  try {
    ...
    if (tables.has('staff_attendance_reconciliation_audit') && tables.has('staff_leave')) await orphan('staff_attendance_reconciliation_audit.leave_request_id', 'SELECT COUNT(*) AS orphan_count FROM staff_attendance_reconciliation_audit a LEFT JOIN staff_leave l ON l.id = a.leave_request_id WHERE l.id IS NULL');
    if (tables.has('student_fee_ledger') && tables.has('student_fee_accounts')) await orphan('student_fee_ledger.account_id', 'SELECT COUNT(*) AS orphan_count FROM student_fee_ledger l LEFT JOIN student_fee_accounts a ON a.id = l.account_id WHERE a.id IS NULL');
