...
The currently active attendance repository reads and writes school scope, student/staff identifiers, class or staff identifiers, attendance date, status, reason, academic year, term, subject key, attendance type, leave linkage, notes, provenance actors, timestamps, and source fields. The reconciliation migration adds only deterministic defaults for existing attendance rows: `UNSPECIFIED` for unknown academic scope, `daily` for unknown student subject scope, `PRESENT` and `MANUAL` for staff attendance defaults, and nullable provenance fields where the source record does not provide a deterministic value.
