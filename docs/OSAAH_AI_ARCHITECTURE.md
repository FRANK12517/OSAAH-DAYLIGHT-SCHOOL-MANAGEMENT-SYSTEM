...
The Financial Intelligence service reads the existing Fee Hub's invoices, payments/receipts, receipt state changes, standalone income, expenses, discounts, and reporting-period identifiers. Fee Hub remains the source of truth and emits revision events after liability, payment, refund/reversal, income, expense, and fee-structure changes. Those events invalidate only cached narration; deterministic snapshots are recomputed from current records and the dashboard refreshes independently of the provider.

All monetary aggregation converts Ghana cedis to integer pesewas before addition and converts back only at the response boundary. Expected revenue is the total of valid issued/part-paid/paid invoices after discounts. Collected is valid payments only; receipt identifiers are not separate revenue. Outstanding is invoice total less valid payments. Expenses and non-fee income include valid standalone records. Refunds/reversals/cancellations are excluded from collected; refunds remain disclosed as a separate metric. Net position is valid collections plus standalone income minus valid expenses, and is withheld whenever quality is not verified complete.

Every source collection is school-scoped and passes Production Data Guard before period filtering or calculation. Failed, cancelled, reversed, TEST, DEMO, SEED, development, and uncertain legacy records cannot contribute. A missing period assignment or excluded source makes the result PARTIAL, retains available verified metrics, and prevents a definitive net-position claim. The default period comes from current School Context when that adapter is configured; authorized historical year/term selections and two-period comparisons remain explicit.

Five Finance tools expose status, collections, outstanding balances, expenses, and period comparison as registered production-only READ/ANALYZE operations. The provider can explain their minimized outputs but never supplies or replaces a calculated figure. Finance dashboard access is explicitly limited to Proprietor/Proprietress, Headteacher, Assistant Headteacher, and Accountant/Bursar with `finance.read`. The Assistant Headteacher receives that minimal read permission; no financial write permission is added.

## Academic and Attendance Intelligence

Academic Intelligence reads the existing Academic Results service and active student/subject configuration. It never recalculates or writes official grades, remarks, positions, signatures, publication state, or result transitions. Production-qualified score rows provide verified student, class, and subject averages, extrema, distribution, strongest/weakest subjects, individual performance, and two-period comparisons. Expected student-subject entries establish completion; an absent score row is missing evidence, never a zero. Teacher requests remain constrained to current assigned classes and subjects.

Attendance Intelligence reads the existing student and staff attendance records and approved staff leave. Existing meanings for present, absent, late/tardy, excused absence, sick absence, early departure, check-in, and approved leave remain distinct. Percentages use submitted attendance rows only. Expected enrolment for a requested day establishes submission completion; a missing submission is never converted into an absence. Daily trends and deterministic two-period comparisons are available without changing attendance records.

Both engines apply Production Data Guard before aggregation, so TEST, DEMO, SEED, development, migration-validation, and uncertain legacy records cannot contribute. Missing or excluded records produce PARTIAL quality and visible warnings. Combined analysis independently authorizes both capabilities and emits only explainable `ACADEMIC_REVIEW_RECOMMENDED` and `ATTENDANCE_REVIEW_RECOMMENDED` indicators. It explicitly does not establish causation or produce an opaque risk score.

The dashboard summary is restricted to Proprietor/Proprietress, Headteacher, Assistant Headteacher, and assignment-scoped Teachers. Deterministic metrics remain visible when narration is unavailable. Optional result comments exist only as non-published AI drafts marked for teacher review and human approval; they do not connect to result publication or overwrite existing teacher comments.

## Admissions and Workforce Intelligence

Admissions Intelligence reads the existing admission-form applications, required fields, attached documents, class/year/term selections, review states, accepted offers, and explicit student attachment that confirms enrollment. It reports completeness, missing requirements, pipeline counts, class/year/month trends, review queues, and application-to-enrollment conversion without changing the admission workflow. Acceptance never equals enrollment unless the authoritative workflow has attached a student record. Class capacity remains unavailable unless an explicit server-owned capacity configuration is supplied.

Duplicate analysis normalizes only the minimum matching attributes needed to label pairs as `EXACT_MATCH`, `STRONG_MATCH`, or `POSSIBLE_MATCH`. Output contains application references, matched attribute categories, and a human-review requirement—not guardian contact details or a decision. The engine never merges, approves, rejects, scores suitability, or enrolls an applicant.

Workforce Intelligence reads existing Staff Management profiles, employment state, role and department, class/subject assignments, staff attendance, approved leave, qualifications, and staff document expiry metadata. It reports active/inactive and teaching/non-teaching counts, role/department/employment trends, incomplete profiles, unassigned staff, document status, attendance, approved leave, and descriptive assignment counts. Coverage gaps are computed only against an explicit requirement configuration; without one, no staffing threshold or overload conclusion is invented.

Both engines require their existing admissions or staff permission independently and pass source records through Production Data Guard. Dashboard responses are aggregate-first and exclude guardian contacts, private staff addresses, confidential fields, credentials, and document contents. Neutral indicators—`ADMISSION_REVIEW_REQUIRED`, `STAFFING_REVIEW_REQUIRED`, and `DOCUMENT_REVIEW_REQUIRED`—require human review. Provider failure removes only narration; verified metrics and quality warnings remain available.
## Dynamic Operational Intelligence

Operational Intelligence discovers actual modules from the OSAAH module/sidebar registry and attaches deterministic analytics providers at runtime. The current production-backed providers cover Transport, Hostel, Inventory, Assets, Procurement, Property/Maintenance, and authorized Communication activity. Missing providers return `UNSUPPORTED`; disabled modules remain `DISABLED` and never expose dashboard cards.

Each provider declares its capability, metrics, permissions, health, production-data policy, data-quality rules, dashboard metrics, exception rules, and deterministic calculation functions. Requests are independently authorized per capability, filtered through the Production Data Guard, assessed by the Data Quality Guard, and exposed only as READ/ANALYZE tools. Capacities, thresholds, due dates, budgets, and causal explanations remain unavailable unless explicitly configured in the source module. Neutral `OPERATIONAL_REVIEW_REQUIRED` indicators never trigger corrective actions.

Future modules default to `ai.enabled = false`. Once validated, their manifest and analytics provider can register navigation, RBAC, tools, metrics, quality policy, dashboard intelligence, and exceptions without changes to Gateway, Orchestrator, or other AI-core routing. See `HOW_TO_ADD_A_FUTURE_OSAAH_AI_ENABLED_MODULE.md`.
## School Knowledge Intelligence

School Knowledge Intelligence maintains a lightweight in-memory, metadata-preserving index of explicitly authoritative OSAAH sources. Current adapters cover published Admission Prospectus content, authorized Official Documents, authorized Academic Calendar events, and the configured school profile. Arbitrary compliance uploads are not indexed as policy because their service stores secure metadata/checksums rather than retrievable approved text.

Only ACTIVE, authoritative, production-provenance sources are chunked. Retrieval rechecks authenticated school scope and the source permission scope, filters inactive versions, returns concise source references, and reports `UNAVAILABLE` when no current source supports an answer. Secret-bearing content is rejected. Unresolved active-source conflicts degrade quality and are surfaced for human review; when stored version or effective-date metadata resolves precedence, the deterministic newest source wins. Provider failure never removes deterministic evidence.
## Executive Brief and Proactive Intelligence
